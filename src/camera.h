...
static inline ray get_ray(const camera *cam, point3 pixel_center,
  ...
  point3 pixel_sample = {
      ...
      .y = pixel_center.y + (offset_x * cam->pixel_delta_x.y) +
           (offset_y * cam->pixel_delta_y.y),
      .z = pixel_center.z + (offset_x * cam->pixel_delta_x.z) +
           (offset_y * cam->pixel_delta_y.z)};
  ...
  scalar ray_time = random_double(rng);
