...
void camera_init(camera *orig_cam) {
  ...
  temp_cam.image_height =
      (temp_cam.image_height < 1) ? 1 : temp_cam.image_height;
  temp_cam.pixel_samples_scale = 1.0 / temp_cam.samples_per_pixel;

  temp_cam.centre = temp_cam.lookfrom;
  // scalar focal_len = vec3_length(vec3_sub(temp_cam.lookfrom,
  // temp_cam.lookat));
  ...
  scalar viewport_width = viewport_height * (((scalar)temp_cam.image_width) /
                                             temp_cam.image_height);
  ...
  vec3 viewport_upper_left = vec3_sub(
      vec3_sub(temp_cam.centre, vec3_scale(temp_cam.w, temp_cam.focus_dist)),
      vec3_add(vec3_scale(viewport_x, 0.5), vec3_scale(viewport_y, 0.5)));
  temp_cam.pixel00_loc = vec3_add(
      viewport_upper_left,
      vec3_scale(vec3_add(temp_cam.pixel_delta_x, temp_cam.pixel_delta_y),
                 0.5));

  scalar defocus_radius =
      temp_cam.focus_dist * tan(degrees_to_radians(temp_cam.defocus_angle / 2));
...
#pragma omp parallel for schedule(dynamic, 1)
  for (int i = 0; i < cam->image_height; i++) {
    ...
    vec3 row_start =
        vec3_add(cam->pixel00_loc, vec3_scale(cam->pixel_delta_y, i));
    pcg32_random_t local_rand;
    pcg32_seed(&local_rand, 42 + i, i);
    for (int j = 0; j < cam->image_width; j++) {

      /*point3 pixel_centre = vec3_add(cam->pixel00_loc,
      vec3_add(vec3_scale(cam->pixel_delta_x, (scalar)j),
      vec3_scale(cam->pixel_delta_y, (scalar)i))); vec3 ray_direction =
      vec3_sub(pixel_centre, cam->centre); ray r = {cam->centre, ray_direction};
      colour pixel_colour = ray_colour(&r, (hittable*) world);*/
      colour pixel_colour = {0, 0, 0};
      vec3 pixel_centre =
          vec3_add(row_start, vec3_scale(cam->pixel_delta_x, j));
      for (int sample = 0; sample < cam->samples_per_pixel; sample++) {
        ray r = get_ray(cam, pixel_centre, &local_rand);
        pixel_colour =
            vec3_add(pixel_colour, ray_colour(&r, (hittable *)world,
                                              cam->max_depth, &local_rand));
      }
      colour_write(buffer, vec3_scale(pixel_colour, cam->pixel_samples_scale),
                   &buffer->buffer[i * cam->image_width + j]);
    }
  }
...
colour ray_colour(ray *r, const hittable *world, int depth,
  ...
  if (world->hit(world, r, (interval){0.0001, infinity}, &rec)) {
    ...
    if (rec.mat->scatter(rec.mat, r, &rec, &attentuation, &scattered, rng)) {
      return vec3_mult(ray_colour(&scattered, world, depth - 1, rng),
                       attentuation);
    }
    return black;
  ...
  return vec3_add((colour){(1.0 - a) * 1, (1.0 - a) * 1, (1.0 - a) * 1},
                  (colour){0.5 * a, 0.7 * a, 1.0 * a});
}
...
/*ray get_ray(const camera* cam, int i, int j) {
    ...
    vec3 pixel_sample = vec3_add(cam->pixel00_loc,
                        vec3_add(vec3_scale(cam->pixel_delta_x, (i + offset.x)),
                                    vec3_scale(cam->pixel_delta_y, (j +
offset.y)))); point3 ray_origin = cam->centre; vec3 ray_direction =
vec3_sub(pixel_sample, ray_origin); return (ray){ray_origin, ray_direction};
}*/
...
point3 defocus_disk_sample(const camera *cam, pcg32_random_t *rng) {
  ...
  return vec3_add(cam->centre, vec3_add(vec3_scale(cam->defocus_disk_x, p.x),
                                        vec3_scale(cam->defocus_disk_y, p.y)));
}
